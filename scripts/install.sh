...
# Register root skill globally
mkdir -p "${SKILLS_DIR}"
cp "${INSTALL_DIR}/src/skill.md" "${SKILLS_DIR}/algernon.md"

# Register sub-skills globally
for skill_dir in "${INSTALL_DIR}/src/skills/"*/; do
  skill_name=$(basename "${skill_dir}")
  mkdir -p "${SKILLS_DIR}/${skill_name}"
  cp "${skill_dir}/SKILL.md" "${SKILLS_DIR}/${skill_name}/SKILL.md"
done

# Create materials and files directories
mkdir -p "${INSTALL_DIR}/materials"
